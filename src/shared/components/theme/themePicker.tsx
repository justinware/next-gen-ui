...
import {
  Colour, ColourPalette, MuiColourPalette, ColourWeight, KeyValuePair
} from '../../types';
import styles from './themePicker.module.css';
import {
  DARK_COLOUR_VARIANT, LIGHT_COLOUR_VARIANT, COLOUR_WEIGHT_200, COLOUR_WEIGHT_300
} from '../../constants';
    ...
    return (
      <div className={paletteRow} key={`${palette.key}-palette-row`}>
        { weights.map(wp => {
          ...
          const isSelected = (
            selectedColour.palette === palette.key
            && Number(selectedColour.weight) === Number(wp.key)
          );
          const backgroundColor = wp.value;
          const color = theme.palette.getContrastText(backgroundColor);
          const disabled = (
            (theme.palette.mode === DARK_COLOUR_VARIANT && Number(wp.key) > COLOUR_WEIGHT_200)
            || (theme.palette.mode === LIGHT_COLOUR_VARIANT && Number(wp.key) < COLOUR_WEIGHT_300)
          );
